  ...
  apiCall(jsonBody, apiUrl, fun) {
    this.Module4Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true) {
          if (fun == "start") {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 4.13', window.localStorage.getItem('username'), 10);
            ...
            window.localStorage.setItem('source', 'module 4.14');
