    ...
    if (e) {
      ...
      if (moduleJson["children"].length != 0) {
        var index1 = moduleJson["children"].findIndex(
          item => item.source == "module 4.9.1");
        if (moduleJson["children"][index1].url != "" && moduleJson["children"][index1].url != null && moduleJson["children"][index1].url != undefined) {
          this.parentUrlJson['4.9.1'] = moduleJson["children"][index1].url;
        }
      }
      this.parentUrlJson['4.9.2'] = e['url'];
      ...
      var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module4.subMenu4-9'), "next": this.translate.instant('L2Module4Finish.subMenu4-10'), "nextRoute": "/modules/module4/Module4.10" }
      ...
      .subscribe(
        data => {
          if (fun == 'start') {
            ...
            this.parentUrlJson["4.9.1"] = this.passUrl;
            var current4 = [];
            current4 = JSON.parse(window.localStorage.getItem("currentJson4"));
            var index = current4["children"].findIndex(
              item => item.source == "module 4.9");
