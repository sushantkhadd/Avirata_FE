...
@Component({
  selector: 'app-module0-20',
  templateUrl: './module0-20.component.html'
})
export class Module020Component implements OnInit {
  ...
  public passData = {}; 
  ngOnInit() {
    if (this.mainFlagModule0 == 20){

    }else if (this.mainFlagModule0 > 20)
    {
      var urlJson = {};
  ...
  apiCall(jsonBody, apiUrl, fun) {
    this.Module0Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true) {
          if (fun == "start") {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 0.20', window.localStorage.getItem('username'), 10);
            ...
            window.localStorage.setItem('mainFlagModule0', '21');
            window.localStorage.setItem('subFlagModule0', '1');
            window.localStorage.setItem('source', 'module 0.21');
            this.Module0Service.setLocalStorage0(3);
            var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module0.subMenu0-21'), "next": this.translate.instant('L2Module0Finish.subMenu0-21'), "nextRoute": "/modules/module0/Module0.21" }
