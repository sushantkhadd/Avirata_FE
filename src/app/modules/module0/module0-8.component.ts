...
@Component({
  selector: 'app-module0-8',
  templateUrl: './module0-8.component.html'
})
export class Module08Component implements OnInit {
  ...
  apiCall(jsonBody, apiUrl, fun) {
    this.Module0Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true) {
          if (fun == "start") {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 0.8', window.localStorage.getItem('username'), 10);
            ...
            window.localStorage.setItem('source', 'module 0.9');
            this.Module0Service.setLocalStorage0(3);
            var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module0.subMenu0-9'), "next": this.translate.instant('L2Module0Finish.subMenu0-9'), "nextRoute": "/modules/module0/Module0.9" }
