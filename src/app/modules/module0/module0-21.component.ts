...
@Component({
  selector: 'app-module0-21',
  templateUrl: './module0-21.component.html'
})
export class Module021Component implements OnInit {
  ...
  ngOnInit() {
    ...
    }else if (this.mainFlagModule0 > 21)
    {
      var urlJson = {};
  ...
  apiCall(jsonBody, apiUrl, fun) {
    this.Module0Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true) {
          if (fun == "start") {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 0.21', window.localStorage.getItem('username'), 10);
            ...
            window.localStorage.setItem('mainFlagModule0', '22');
            window.localStorage.setItem('subFlagModule0', '1');
            window.localStorage.setItem('source', 'module 0.22');
            this.Module0Service.setLocalStorage0(3);
            var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module0.subMenu0-22'), "next": this.translate.instant('L2Module0Finish.subMenu0-22'), "nextRoute": "/modules/module0/Module0.22" }
