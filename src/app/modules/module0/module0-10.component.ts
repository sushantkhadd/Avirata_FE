...
@Component({
  selector: 'app-module0-10',
  templateUrl: './module0-10.component.html'
})
export class Module010Component implements OnInit {
  ...
  ngOnInit() {
    ...
    this.startPdf=false
    if (this.mainFlagModule0 == 10) {
      this.start();
    }
    else if (this.mainFlagModule0 > 10) {
      var urlJson = {};
      urlJson = JSON.parse(window.localStorage.getItem("currentJson0"));
      console.log("vcxxxx", urlJson);
      if (urlJson["children"].length > 0) {
        var index = urlJson["children"].findIndex(
          item => item.source == "module 0.10"
        );
  ...
  apiCall(jsonBody, apiUrl, fun) {
    this.Module0Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true) {
          if (fun == "start") {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 0.10', 
            window.localStorage.getItem('username'), 10);
            ...
            window.localStorage.setItem('mainFlagModule0', '11');
            window.localStorage.setItem('subFlagModule0', '1');
            window.localStorage.setItem('source', 'module 0.11');
            this.Module0Service.setLocalStorage0(3);
            var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module0.subMenu0-11'), "next": this.translate.instant('L2Module0Finish.subMenu0-11'), "nextRoute": "/modules/module0/Module0.11" }
