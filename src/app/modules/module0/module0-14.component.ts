...
@Component({
  ...
  templateUrl: './module0-14.component.html'
})
export class Module014Component implements OnInit {
  ...
  apiCall(jsonBody, apiUrl, fun) {
    this.Module0Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true) {
          if (fun == "start") {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 0.14', window.localStorage.getItem('username'), 10);
            ...
            window.localStorage.setItem('mainFlagModule0', '15');
            window.localStorage.setItem('subFlagModule0', '1');
            window.localStorage.setItem('source', 'module 0.15');
            this.Module0Service.setLocalStorage0(3);
            var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module0.subMenu0-15'), "next": this.translate.instant('L2Module0Finish.subMenu0-15'), "nextRoute": "/modules/module0/Module0.15" }
