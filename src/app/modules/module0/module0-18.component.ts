...
@Component({
  selector: 'app-module0-18',
  templateUrl: './module0-18.component.html'
})
export class Module018Component implements OnInit {
  ...
  public passData = {}; 
  ngOnInit() {
    if (this.mainFlagModule0 == 18){
this.start()
    }else if (this.mainFlagModule0 > 18)
    {
      var urlJson = {};
  ...
  apiCall(jsonBody, apiUrl, fun) {
    this.Module0Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true) {
          if (fun == "start") {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 0.18', window.localStorage.getItem('username'), 10);
            ...
            window.localStorage.setItem('mainFlagModule0', '19');
            window.localStorage.setItem('subFlagModule0', '1');
            window.localStorage.setItem('source', 'module 0.19');
            this.Module0Service.setLocalStorage0(3);
            var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module0.subMenu0-19'), "next": this.translate.instant('L2Module0Finish.subMenu0-19'), "nextRoute": "/modules/module0/Module0.19" }
