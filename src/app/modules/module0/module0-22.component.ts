...
@Component({
  selector: 'app-module0-22',
  templateUrl: './module0-22.component.html'
})
export class Module022Component implements OnInit {
  ...
  apiCall(jsonBody, apiUrl, fun) {
    ...
    this.Module0Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true) {
          if (fun == "start") {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 0.22', window.localStorage.getItem('username'), 10);
            ...
            window.localStorage.setItem('mainFlagModule0', '23');
            window.localStorage.setItem('subFlagModule0', '1');
            window.localStorage.setItem('source', 'module 0.23');
            this.Module0Service.setLocalStorage0(3);
            var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module0.subMenu0-23'), "next": this.translate.instant('L2Module0Finish.subMenu0-23'), "nextRoute": "/modules/module0/Module0.23" }
