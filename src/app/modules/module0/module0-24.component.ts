...
@Component({
  selector: 'app-module0-24',
  templateUrl: './module0-24.component.html'
})
export class Module024Component implements OnInit {
  ...
  public passData = {}; 
  ngOnInit() {
    if (this.mainFlagModule0 == 24){

    }else if (this.mainFlagModule0 > 24)
    {
      var urlJson = {};
  ...
  apiCall(jsonBody, apiUrl, fun) {
    this.Module0Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true) {
          if (fun == "start") {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 0.24', window.localStorage.getItem('username'), 10);
            ...
            window.localStorage.setItem('mainFlagModule0', '25');
