...
@Component({
  selector: 'app-module0-23',
  templateUrl: './module0-23.component.html'
})
export class Module023Component implements OnInit {
  ...
  public passData = {}; 
  ngOnInit() {
    if (this.mainFlagModule0 == 23){

    }else if (this.mainFlagModule0 > 23)
    {
      var urlJson = {};
  ...
  apiCall(jsonBody, apiUrl, fun) {
    this.Module0Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true) {
          if (fun == "start") {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 0.23', window.localStorage.getItem('username'), 10);
            ...
            window.localStorage.setItem('mainFlagModule0', '24');
            window.localStorage.setItem('subFlagModule0', '1');
            window.localStorage.setItem('source', 'module 0.24');
            this.Module0Service.setLocalStorage0(3);
            var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module0.subMenu0-24'), "next": this.translate.instant('L2Module0Finish.subMenu0-24'), "nextRoute": "/modules/module0/Module0.24" }
