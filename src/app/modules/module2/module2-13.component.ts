...
export class Module213Component implements OnInit {
  ...
  finishCFU(result) {
    if (result["status"] == true)
    {
      ...
      var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module2.subMenu2-13'), "next": this.translate.instant('L2Module2Finish.subMenu2-14'), "nextRoute": "/modules/module2/Module2.14" }
    ...
    this.Module2Service.apiCall(jsonBody, apiUrl)
      .subscribe(
        data => {
          if (fun == 'start')
          {
            this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 2.13', window.localStorage.getItem('username'), 10);
