...
export class CoordinatorTransferComponent implements OnInit {
  ...
  createNewUser() {
    ...
    if (this.aadharNumber == null || this.aadharNumber == undefined) {
      ...
      jsonBody['aadharno'] = this.aadharNumber.trim();
    }
    if (this.underGraduate === 'other') {
      jsonBody['undergraduation'] = this.otherUgSpecial.trim();
    } else {
      jsonBody['undergraduation'] = this.underGraduate;
    }

    if (this.postGraduate === 'pgother') {
      jsonBody['postgraduation'] = this.otherPgSpecial.trim();
    } else {
      jsonBody['postgraduation'] = this.postGraduate;
    }

    jsonBody['ugspecial'] = this.ugSpecialisation.trim();
    jsonBody['pgspecial'] = this.pgSpecialisation.trim();
    jsonBody['professional'] = this.professional;
    jsonBody['professionalspecil'] = this.professionalSpecialization.trim();
    jsonBody['other'] = this.othersQualification.trim();
    jsonBody['accno'] = this.accountNo.trim();
    jsonBody['ifsccode'] = this.ifscCode.trim();
    jsonBody['bankname'] = this.bankName.trim();
