...
@Component({
  selector: 'app-module5-11',
  templateUrl: './module5-11.component.html'
})
export class Module511Component implements OnInit {
  ...
  start() {
    ...
    this.Module5Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        if (data["status"] == true)
        {
          this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 5.11', window.localStorage.getItem('username'), 10);
    ...
    this.Module5Service.apiCall(jsonBody, apiUrl).subscribe(
      data => {
        ...
        )
        {
          ...
          window.localStorage.setItem('source', 'module 5.12.1');
          var obj = {
            "type": "submodule",
            "route": true,
            "current": this.translate.instant("L2Module5.subMenu5-11"),
            "next": this.translate.instant("L2Module5Finish.subMenu5-12"),
            "nextRoute": "/modules/module5/Module5.12"
          };
          this.LocalstoragedetailsService.setModuleStatus(JSON.stringify(obj));
          this.Module5Service.setLocalStorage5(12);
        }
      },
      error => {
        this.LanguageService.handleError(error.error.message);
