...
export class Module5Component implements OnInit {
  ...
  finishCFU(e) {
    if (e) {
      ...
      item => item.source == "module 5.1.1");
      if(moduleJson["children"][index1].url !="" && moduleJson["children"][index1].url !=null && moduleJson["children"][index1].url !=undefined){
        this.parentUrlJson['5.1.1'] = moduleJson["children"][index1].url;
      }
      }
      this.parentUrlJson['5.1.2'] = e['url'];
      ...
      var obj = { "type": "submodule", "route": true, "current": this.translate.instant('L2Module5.subMenu5-1'), "next": this.translate.instant('L2Module5Finish.subMenu5-2'), "nextRoute": "/modules/module5/Module5.2" }
  ...
  apiCall(jsonBody, apiUrl, fun) {
    this.Module5Service.apiCall(jsonBody, apiUrl)
      ...
      data => {
        if (fun == 'start') {
          this.LanguageService.googleEventTrack('L3SubmoduleStatus', 'Module 5.1', window.localStorage.getItem('username'), 10);
