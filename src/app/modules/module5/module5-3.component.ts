...
export class Module53Component implements OnInit {
  ...
  ngOnInit() {
    ...
    } else if (this.mainFlagModule5 > 3)
    {
      ...
      if (urlJson["children"].length > 0)
      {
        ...
        if (mainJson != null)
        {
          this.urlArray["src1"] = mainJson["5.3.1"];
          this.urlArray["src2"] = mainJson["5.3.2"];
          this.urlArray["src3"] = mainJson["5.3.3"];
        }
      }
    }
  }
  ...
  checkAnswer(e) {
    ...
    console.log(e, "currentJson5");
    this.audioSrc["state"] = "static";
    let dummyUrl = {};
    let dummyData = JSON.parse(e["url"]);
    dummyUrl["5.3.1"] = dummyData["1"];
    dummyUrl["5.3.2"] = dummyData["2"];
    dummyUrl["5.3.3"] = dummyData["3"];
    ...
    window.localStorage.setItem("source", "module 5.4");
    this.Module5Service.setLocalStorage5(4);
    var obj = {
      type: "submodule",
      route: true,
      current: this.translate.instant("L2Module5.subMenu5-3"),
      next: this.translate.instant("L2Module5.subMenu5-4"),
